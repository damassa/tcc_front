...
export const getCategory = async (): Promise<CategoryResponse[]> => {
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/categories`, getAxiosConfig());
...
export const addCategory = async (category: Category): Promise<CategoryResponse> => {
    const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/series`, category, getAxiosConfig());
    return res.data;
}
